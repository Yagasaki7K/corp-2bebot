  ...
  watch: {
    response: function () {
      ...
      const totalNames = (names || []).map((item) => {
        return {
          percentage: (
            ((this.response.res2 || []).find((item2) => item2.rank === item)
              ?.amount /
              totalTransfers) *
            100
          ).toFixed(2),
      ...
      const resTotalPercentage = {
        label: "Percentual de Transferências",
        data: [...totalNames.map((item) => item.percentage)],
        // data: [totalSells, ...totalNames.map((item) => item.sells)],
        backgroundColor: "#A4DBE8",
      };

      const resTotal = [resTotalPercentage];
